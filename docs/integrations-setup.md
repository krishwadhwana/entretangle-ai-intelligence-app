...
| **Stripe** | No platform app — paste a **restricted API key** (Developers → API keys → Create restricted key → **Charges: Read**). Use `rk_live_…` for real revenue, `rk_test_…` for test data. | — (per-account key) | — |
