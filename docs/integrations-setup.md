...
| **Stripe** | [dashboard.stripe.com/settings/connect](https://dashboard.stripe.com/settings/connect) (enable Connect). Secret key from [dashboard.stripe.com/apikeys](https://dashboard.stripe.com/apikeys). | `STRIPE_CLIENT_ID` (the `ca_…` Connect id), `STRIPE_SECRET_KEY` | `…/callback/stripe` |
